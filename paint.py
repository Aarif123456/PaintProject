...
def renderToolbox(toolClassNum):
    for i in range(len(toolsName[toolClassNum])):
        ...
        addImageNonSmooth(screen, (80, 70), toolBoxRect[i],
                          f"{toolBoxPath}{toolsClassName[toolClassNum]}/{i + 1}_{toolsName[toolClassNum][i]}.png")
...
def renderToolInfoText(x, y, toolClassNum, toolName):
    ...
    PaintLayout.drawText(screen, open(
        toolBoxPath + toolsClassName[toolClassNum] + "/" + str(curTool + 1) + "_" + toolsName[toolClassNum][
            curTool] + ".txt").read().splitlines()[0], (0, 0, 0), toolboxTextInfoRect, 17)
...
def renderSizeRect(size):
    fullSizeRect = Rect(5, 677, 185, 30)  # the entire size hoverRect
...
# background box
# the actual background has to be in hoverRect with ratio 17:12
