    ...
    def ellipse(self, surface, col, fill, canvasRect, coord, perfect, size):
        ...
        if perfect:  # perfect use circle
            if (abs(mx - startx) + abs(my - starty)) // 2 > size:
                draw.circle(self.screen, col, (startx, starty), ((abs(mx - startx) + abs(my - starty)) // 2),
                            size * fill)
            else:
            ...
            if abs(mx - startx) > size * 2 and abs(my - starty) > size * 2:
                draw.ellipse(self.screen, col, (min(mx, startx), min(my, starty), abs(mx - startx), abs(my - starty)),
                             size * fill)
            else:
                draw.ellipse(self.screen, col, (min(mx, startx), min(my, starty), abs(mx - startx), abs(my - starty)),
                             0)
